...
do_wget() {
  ...
  if [ -z "$2" ]; then
    wget --user-agent="User-Agent: mixlib-install/3.16.3" --content-disposition "$1" 2>$tmp_dir/stderr
  else
    wget --user-agent="User-Agent: mixlib-install/3.16.3" -O "$2" "$1" 2>$tmp_dir/stderr
  fi
  rc=$?
  # check for 404
...
do_curl() {
  ...
  if [ -z "$2" ]; then
    curl -A "User-Agent: mixlib-install/3.16.3" --retry 5 -sL -D $tmp_dir/stderr -O -J "$1"
    rc=$?
  else
    curl -A "User-Agent: mixlib-install/3.16.3" --retry 5 -sL -D $tmp_dir/stderr "$1" > "$2"
    rc=$?
  fi
...
do_fetch() {
  echo "trying fetch..."
  fetch --user-agent="User-Agent: mixlib-install/3.16.3" -o "$2" "$1" 2>$tmp_dir/stderr
  # check for bad return status
  [ $? -ne 0 ] && return 1
  return 0
}
...
do_python() {
  echo "trying python..."
  python -c "import sys,urllib2; sys.stdout.write(urllib2.urlopen(urllib2.Request(sys.argv[1], headers={ 'User-Agent': 'mixlib-install/3.16.3' })).read())" "$1" > "$2" 2>$tmp_dir/stderr
  rc=$?
  # check for 404
...
#
#  EXAMPLE
#    curl -L 'https://chefdownload-commercial.chef.io/install.sh' | sudo bash -s --  -P chef -c stable
#
# Gets the download url and SHA256 checksum for the latest stable release of Chef Workstation.
#   EXAMPLE
#     curl -L 'https://chefdownload-commercial.chef.io/install.sh' | sudo bash -s --  -P chef-workstation -c stable
#
# Inputs:
# $base_api_url:
# $channel:
...
if [ -z "$download_url_override" ]; then
  ...
  if [ -z "$base_api_url" ]; then
    if [ -n "$license_id" ]; then
      ...
      # Omnitruck endpoint
      base_api_url="https://omnitruck.chef.io"
    fi
  fi
