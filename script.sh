...
do_wget() {
  ...
  if test "x$2" = "x"; then
    wget --user-agent="User-Agent: mixlib-install/3.13.0" --content-disposition "$1" 2>$tmp_dir/stderr
  else
    wget --user-agent="User-Agent: mixlib-install/3.13.0" -O "$2" "$1" 2>$tmp_dir/stderr
  fi
  rc=$?
  # check for 404
...
do_curl() {
  ...
  if test "x$2" = "x"; then
    curl -A "User-Agent: mixlib-install/3.13.0" --retry 5 -sL -D $tmp_dir/stderr -O -J "$1"
    rc=$?
  else
    curl -A "User-Agent: mixlib-install/3.13.0" --retry 5 -sL -D $tmp_dir/stderr "$1" > "$2"
    rc=$?
  fi
...
do_fetch() {
  echo "trying fetch..."
  fetch --user-agent="User-Agent: mixlib-install/3.13.0" -o "$2" "$1" 2>$tmp_dir/stderr
  # check for bad return status
  test $? -ne 0 && return 1
  return 0
}
...
do_python() {
  echo "trying python..."
  python -c "import sys,urllib2; sys.stdout.write(urllib2.urlopen(urllib2.Request(sys.argv[1], headers={ 'User-Agent': 'mixlib-install/3.13.0' })).read())" "$1" > "$2" 2>$tmp_dir/stderr
  rc=$?
  # check for 404
